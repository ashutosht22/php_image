...
//imagecopyresized($stamp, $stamp1, 0, 0, 0,   0,  ($h/20), ($h/20),$w1, $h1);
  ...
  $text_color = imagecolorallocate($new, $textcolorR,$textcolorG,$textcolorB);
   	...
   	//  $upbar = imagecreatefrompng('upbar.png');
        ...
        $upbarh1=$w;
	//	echo "upbar". $upbarh;
	  	$upbar=@imagecreate($upbarw1,$upbarh1) or die("Cannot Initialize new GD im age stream");
		 $upbar2 = imagecolorallocatealpha($upbar, 255,249,81, 55);
		  imagefill($upbar, 0, 0, $upbar2);

imagecopy($new, $upbar, 180, 110, 0, 0, imagesx($upbar), imagesy($upbar));
echo $h;
...
$text="One of the best digital marketing software in the market.One of the best digital marketing software in the market.";
 ...
 imagettftext($new, 26, 0, 300, 550, $text_color, $font2, $newtext);
 }else{
	 $newtext = wordwrap($text2, 30, " \n");  
 imagettftext($new, 22, 0, 300, 550, $text_color, $font2, $newtext);
 }
  imagettftext($new, 28, 0, 320, 220, $text_color, $font1, "Arkbel Innovations Pvt. Ltd");
   imagettftext($new, 42, 0, 375, 440, $text_color, $font1, "REVIEW ");
  // imagettftext($new, 28, 0, 30, 480, $text_color, $font1, "ABOUT Arkbel Innovations Pvt. Ltd ");
    imagettftext($new, 28, 0, 320, 950, $text_color, $font1, "Ashutosh Tiwari");
echo  $newtext;
    // Save the picture
 
        imagejpeg($new, $public_file_path . '/guest3.jpg', 100); 
    imagedestroy($new); 
 }
 ?>